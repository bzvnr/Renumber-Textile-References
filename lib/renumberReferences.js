const { referenceSection, referenceSectionStart, referenceSectionEnd, mainHeading, formatCheckResults } = require("./config");
...
function renumberReferences(text) {
  ...
  text = deleteReferences(text, references);
  ...
  text = deleteReferences(text, uncitedReferences);
  // merge cited and uncited refernces
  references = new Map([...references, ...uncitedReferences]);
  text = recreateReferences(text, references);
  return text;
}

// delete references corresponding to collected inTextCitations
function deleteReferences(text, references) {
...
function deleteReferenceFromText(text, oldCitationNumber) {
  ...
  // go to end of line for the reference
