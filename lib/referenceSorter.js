...
function errorCheck(text, ignoreConfirmCode) {
  ...
  */
  const uRefNums = uniqueReferences.map(ref => ref.substring(fnLength, ref.length - 1));
  const uCitNums = uniqueInTextCitations.map(cit => cit.substring(openBracketLength, cit.length - closeBracketLength));
