...
const { sortReferences } = require("./referenceSorter");
...
function compareActualToExpected(testCase) {
  ...
  const output = sortReferences(input);
...
test("", () => {
  compareActualToExpected(standard.referenceBeforeEndSectionBeforeCitation);
})

test("", () => {
  compareActualToExpected(standard.referenceBeforeEndSectionAfterCitation);
})
